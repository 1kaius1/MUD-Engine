...
// MUD Engine - Main Server Entry Point
...
func (c *Client) handleGameCommand(command string) {
	// Create a player object for the command system
	player := &game.Player{
		ID:            c.playerID,
		Username:      c.username,
...
func main() {
	...
	go func() {
		...
		log.Printf("Listening on: %s", cfg.GetListenAddress())
...
func saveAllPlayerData(server *Server) {
	...
	for client := range server.clients {
		if client.authState == StateAuthenticated {
			// TODO: Save player location, health, inventory, keys, etc.
			// For now, just log
